...
void SimpleSocketServer::re_establish() {
    ...
    if (tmp_fd < 0) {
        ...
        // The server cannot accept a new client because there are none trying to
        // connect. Move on.
        return;
    } else {
        // A new client has connected! Assign the fd field of the base class, and
...
// Close the server fd when deconstructing. Note that the base class
// deconstructor is called automatically, so we do not (and should not) close it
// here.
