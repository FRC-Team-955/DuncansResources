...
RealSenseBGRDFrameSource::RealSenseBGRDFrameSource(cv::Size size, int fps)
    : image_size(size), align(RS2_STREAM_COLOR) {
        rs2::config cfg;
...
BGRDFrame RealSenseBGRDFrameSource::next() {
    ...
    auto output_frame =
        BGRDFrame(cv::Mat(image_size, CV_8UC3), cv::Mat(image_size, CV_16UC1));

    rs2::frameset realsense_frame = align.process(pipe.wait_for_frames());

    memcpy(output_frame.bgr.data,
            (uchar*)realsense_frame.get_color_frame().get_data(),
            output_frame.bgr.elemSize() * output_frame.bgr.total());

    memcpy(output_frame.depth.data,
            (uchar*)realsense_frame.get_depth_frame().get_data(),
            output_frame.depth.elemSize() * output_frame.depth.total());
